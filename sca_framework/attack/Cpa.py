...
from sca_utils.utils import kahanSum, saveObject, intToBytes, bytesToInt, hw
from sca_utils.preprocess import aggregate, highpass
from sca_utils.metrics import rankKey
...
class Cpa():
    ...
    `fromFiles(files, n_traces, chunk_size, *kargs)`:
        Performs CPA attack starting from `files` for all bytes.
    """
    ...
    def _getPowerEstimation(self, plains, byte, keys_byte):
        ...
        return hw[AesSca.attackedSbox(byte)[np.repeat(plain_byte, self._I, axis=1) ^ keys_byte]]
    ...
    def rankKey(self,
        ...
        k = [AesSca.attackedKeyByte(key, i) for i in range(self._nBytes)]
    ...
    def plot(self,
        ...
        r = self.computeCoefficient()
        keyByte = AesSca.attackedKeyByte(key, byte)
    ...
    def fromFile(self,
                 ...
                 *,  # Force to use keyword arguments
                 save: bool = False,
                 path: str = ...) -> tuple[np.ndarray, list[int]]:
        """
        Performs CPA attack starting from `files` for all bytes.
        Attack intermediate intermediate is SBOX(plain ^ key) for all 16 bytes;

        Parameters
        ----------
        `file`     : str
            File path containg the attack traces, in .dat format.
        `n_traces`  : int, optional
            How many traces use to attack (default uses all traces in file).
        `chunk_size`: int, optional
            How many traces process at once (default is 10_000).
        `save`      : bool, optional
            Save the cpa object and results (default is False).
        `path`      : str, optional
            Path where to save the cpa object and results.

        Returns
        ----------
        Pearson's correlation cofficient and rank of the key for each byte.
        """

        self._D = 0  # Force reset
        ...
        # Compute Pearson's correlation cofficient
        r = self.computeCoefficient()
        rank = self.rankKey(traces_bin.getKey())

        if save:
            self.dump(path, bytesToInt(traces_bin.getKey()), ovveride=True)
        return r, rank
