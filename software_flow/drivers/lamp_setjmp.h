/**
 * jarvis_setjmp.h provides non-local jumps. The control flow deviates from
 * the usual subroutine call and return sequence. The complementary fuctions
 * jarvis_setjmp and jarvis_longjmp provide this functionality.
 */


#ifndef JARVIS_SETJMP_H
#define JARVIS_SETJMP_H

#include "jarvis_core.h"
 ...
 * the return is from a call to 'jarvis_longjmp'.
 */
int jarvis_setjmp(tc_t *tc);

/**
 * Restore the context of the environment buffer 'tc' that was previously
 * saved by invoking the 'jarvis_setjmp' routine. After the longjmp is
 ...
 * \return nothing.
 */
void jarvis_longjmp(tc_t *tc, int ret_val);
