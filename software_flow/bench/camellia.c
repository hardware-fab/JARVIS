...
#define GRAND_ROUNDS 3 // key_lenght == 128 ? 3 : 4
...
void Camellia_Ekeygen(const uint8_t *rawKey, KEY_TABLE_TYPE k)
{
...
void Camellia_EncryptBlock_Rounds(const uint8_t plaintext[],
                                  const KEY_TABLE_TYPE keyTable,
                                  uint8_t ciphertext[])
{
    register u32 s0, s1, s2, s3;
    const u32 *k = keyTable, *kend = keyTable + GRAND_ROUNDS * 16;
...
int main()
{
    ...
    for (i = 0; i < BATCH_SIZE; i++)
    {
        ...
        printf(" -> ");
#endif
        Camellia_EncryptBlock_Rounds(state, keyTable, state);
#if ISA == 1
        for (int j = 0; j < 16; j++)
            printf("%02x", state[j]);
        printf("\n");
