// *************************************************
// The source code has been adapted from the following repository:
// https://github.com/openssl/openssl
//
// Copyright 2006-2020 The OpenSSL Project Authors. All Rights Reserved.
// Licensed under the Apache License 2.0 (the "License").  You may not use
// this file except in compliance with the License.  You can obtain a copy
// in the file LICENSE in the source distribution or at
// https://www.openssl.org/source/license.html
// 
// Other contributor(s) :  Andrea Galimberti (andrea.galimberti@polimi.it),
//                         Davide Galli (davide.galli@polimi.it),
//                         Davide Zoni (davide.zoni@polimi.it)
// 
// *************************************************

#include <stdint.h>
#include <trng.h>

// constant execution time (no if instructions)
#define IS_CONST_TIME 1

// #define ISA 0     // Enable simulation exit sequence for riscv-polimi core
// #define ISA 1     // Enable debug mode on X86 machines
#define ISA 2 // For prototype: no x86 nor RISC-V ad-hoc statements

#if ISA == 0
#include "sim_handler.h"
#endif

#if ISA == 1
#include <stdio.h>
#include <stdlib.h>
#include <inttypes.h>
#include <time.h>
#endif

#define BATCH_SIZE 1024

const uint8_t sbox[256] = {
    };

uint8_t rand_sbox[256];
uint8_t xor_sbox[256];

uint8_t rcon[] = {
    0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40,
    0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d, 0x9a};

uint32_t rand32()
{
    uint32_t rng;
#if ISA == 2
    rng = trng();
#else
    rng = (uint32_t)rand(); // Returns a pseudo-random integer
#endif
    return rng;
}

uint8_t gmul(uint8_t a, uint8_t b)
{
    uint8_t i = 0;
    uint8_t p = 0;
    uint8_t hi_bit;
#if IS_CONST_TIME == 1
    // additional var to avoid data-dependant if
    struct
    {
        signed int tVal : 1;
    } s;
    uint8_t modVal = 0;
#endif

    for (i = 0; i < 8; ++i)
    {
#if IS_CONST_TIME == 1
        s.tVal = b & 1; // is b[0] bit set?
        modVal = s.tVal;
        p = p ^ (a & modVal);
#else
        if ((b & 1) == 1)
            p = p ^ a;
#endif
        hi_bit = (a & 0x80);
        a <<= 1;

#if IS_CONST_TIME == 1
        s.tVal = (hi_bit >> 7) & 1; // is hi_bit[7] bit set?
        modVal = s.tVal;
        a = a ^ (0x1b & modVal);
#else
        if (hi_bit == 0x80)
            a = a ^ 0x1b;
#endif
        b >>= 1;
    }
    return p;
}

uint8_t MixColumns(uint8_t *state)
{
    uint8_t i = 0;
    uint8_t s0, s1, s2, s3;

    for (i = 0; i < 16; i += 4)
    {
        s0 = state[i];
        s1 = state[i + 1];
        s2 = state[i + 2];
        s3 = state[i + 3];

        state[i] = gmul(0x02, s0) ^ gmul(0x03, s1) ^ s2 ^ s3;
        state[i + 1] = s0 ^ gmul(0x02, s1) ^ gmul(0x03, s2) ^ s3;
        state[i + 2] = s0 ^ s1 ^ gmul(0x02, s2) ^ gmul(0x03, s3);
        state[i + 3] = gmul(0x03, s0) ^ s1 ^ s2 ^ gmul(0x02, s3);
    }

    return 0;
}

uint8_t SubBytes(uint8_t *state)
{
    register uint8_t init = (uint8_t)(rand32() & 0xFF);
    register uint8_t v1 = rand_sbox[init];
    register uint8_t v2 = xor_sbox[init];
    register uint8_t iold = init;

    register uint8_t l1, l2;
    uint8_t r, mask;

    for (uint8_t i = 0; i < 16; i++)
    {
        l1 = rand_sbox[state[i]];
        l2 = xor_sbox[state[i]];

        r = (rand32() & 0x01) ? 0xFF : 0x00;
        v1 = (l1 & r) | (v1 & ~r);
        v2 = (l2 & r) | (v2 & ~r);
        iold = (state[i] & r) | (iold & ~r);

        mask = (uint8_t)(rand32() & 0xFF);
        rand_sbox[iold] = v1 ^ mask;
        xor_sbox[iold] = v2 ^ mask;

        state[i] = l1 ^ l2;
    }

    return 0;
}

uint8_t ShiftRows(uint8_t *state)
{
    uint8_t i = 0;
    uint8_t s0, s1, s2, s3;

    for (i = 1; i < 4; ++i)
    {
        s0 = state[(i + (4 * i)) % 16];
        s1 = state[(i + (4 * i) + 4) % 16];
        s2 = state[(i + (4 * i) + 8) % 16];
        s3 = state[(i + (4 * i) + 12) % 16];

        state[i] = s0;
        state[i + 4] = s1;
        state[i + 8] = s2;
        state[i + 12] = s3;
    }

    return 0;
}

uint8_t AddRoundKey(uint8_t *state, uint8_t *key)
{
    uint8_t i = 0;

    for (i = 0; i < 16; ++i)
    {
        uint32_t r = (rand32() & 0b111);
        switch (r)
        {
        case 0b000:
            state[i] = (~state[i] & key[i]) | (state[i] & ~key[i]);
            break;
        case 0b001:
            state[i] = (state[i] & key[i]) ^ (state[i] | key[i]);
            break;
        case 0b010:
            state[i] = (state[i] | key[i]) & (~state[i] | ~key[i]);
            break;
        case 0b011:
            state[i] ^= key[i];
            break;
        case 0b100:
            state[i] = (state[i] | key[i]) & ~(state[i] & key[i]);
            break;
        case 0b101:
            state[i] = ~((state[i] | ~key[i]) & (~state[i] | key[i]));
            break;
        case 0b110:
            state[i] = ~((~state[i] & ~key[i]) | (state[i] & key[i]));
            break;
        case 0b111:
            state[i] = (state[i] & ~key[i]) ^ (~state[i] & key[i]);
            break;
        default:
            // Should never happen
            state[i] ^= key[i];
            break;
        }
    }

    return 0;
}

uint8_t ScheduleCore(uint8_t *nextkey, uint8_t *subkey, uint8_t rconi)
{
    uint8_t temp[4];

    // copy over to temp (incl rotation)
    temp[0] = sbox[subkey[13]];
    temp[1] = sbox[subkey[14]];
    temp[2] = sbox[subkey[15]];
    temp[3] = sbox[subkey[12]];

    nextkey[0] = subkey[0] ^ temp[0] ^ rcon[rconi];
    nextkey[1] = subkey[1] ^ temp[1];
    nextkey[2] = subkey[2] ^ temp[2];
    nextkey[3] = subkey[3] ^ temp[3];

    nextkey[4] = subkey[4] ^ nextkey[0];
    nextkey[5] = subkey[5] ^ nextkey[1];
    nextkey[6] = subkey[6] ^ nextkey[2];
    nextkey[7] = subkey[7] ^ nextkey[3];
    nextkey[8] = subkey[8] ^ nextkey[4];
    nextkey[9] = subkey[9] ^ nextkey[5];
    nextkey[10] = subkey[10] ^ nextkey[6];
    nextkey[11] = subkey[11] ^ nextkey[7];
    nextkey[12] = subkey[12] ^ nextkey[8];
    nextkey[13] = subkey[13] ^ nextkey[9];
    nextkey[14] = subkey[14] ^ nextkey[10];
    nextkey[15] = subkey[15] ^ nextkey[11];

    return 0;
}

uint8_t KeyExpansion(uint8_t fullkeys[11][16], uint8_t *key)
{
    uint8_t i;

    for (i = 0; i < 16; ++i)
    {
        fullkeys[0][i] = key[i];
    }

    for (i = 0; i < 10; ++i)
    {
        ScheduleCore(fullkeys[i + 1], fullkeys[i], i + 1);
    }

    return 0;
}

void AES_in_place(uint8_t k[11][16], uint8_t *m)
{
    uint8_t i;
    AddRoundKey(m, k[0]);
    for (i = 0; i < 9; ++i)
    {
        SubBytes(m);
        ShiftRows(m);
        MixColumns(m);
        AddRoundKey(m, k[i + 1]);
    }

    SubBytes(m);
    ShiftRows(m);
    AddRoundKey(m, k[i + 1]);
}

void FillRandSbox()
{
    for (int i = 0; i < 256; i++)
    {
        rand_sbox[i] = (uint8_t)(rand32() & 0xFF);
        xor_sbox[i] = sbox[i] ^ rand_sbox[i];
    }
}

unsigned char key[16] = {0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c};
uint8_t state[16] = {0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41};

int iterDelay = 0;

int main()
{
    int i;
#if ISA == 1
    int j;
#endif
    uint8_t fullkeys[11][16];

    // expand the key to subkeys
    KeyExpansion(fullkeys, key);

    /* Receive initial state and encrypt it BATCH_SIZE times */
    for (i = 0; i < BATCH_SIZE; i++)
    {
        FillRandSbox();

        __asm__(".RISE_TRG:");
        // To avoid ringing effect when the trigger activates
        for (iterDelay = 0; iterDelay < 500; iterDelay++)
            ;

#if ISA == 1
        srand(time(NULL));
        for (j = 0; j < 16; j++)
            printf("%02x", state[j]);
        printf(" -> ");
#endif

        AES_in_place(fullkeys, state);
        __asm__(".FALL_TRG:");

        // To have time to reset trigger after AES execution,
        // otherwise the following execution of AES would not be detected
        for (iterDelay = 0; iterDelay < 9000; iterDelay++)
            ;

#if ISA == 1
        for (j = 0; j < 16; j++)
            printf("%02x", state[j]);
        printf("\n");
#endif
    }
}
